...
echo "Compressing files from remote this might take time if this taking too much of time to output anything then close the programm something is wrong "

sudo ssh -i $key $user@$host "cd $dir;zip -r $newfile.zip ."
echo "Downloading $newfile.zip if this taking too much of time to output anything then close the programm something is wrong "
sudo scp -i $key -r $user@$host:$dir$newfile".zip" .
sudo unzip $newfile".zip"
sudo rm -r $newfile".zip"
sudo ssh -i $key $user@$host "cd $dir;rm -r $newfile.zip"
